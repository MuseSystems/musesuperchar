...
this.MuseSuperChar.LotSerial = this.MuseSuperChar.LotSerial || {};
...
(function(pPublicApi, pGlobal) {
    ...
    var ENTITY_DATA_TABLE = "public_ls";
    //--------------------------------------------------------------------
    //  Get Object References From Screen Definitions
    //--------------------------------------------------------------------
    var qtyToAssignLabel = mywindow.findChild("_qtyToAssignLit");
    var mainGridLayout = mywindow.layout();
    ...
    // Figure out where the last actual form row is.
    var lastFormRow = 0;
    try {
        for (var i = 0; i < mainGridLayout.rowCount(); i++) {}
    } catch (e) {
        var myError = new MuseUtils.ApiException(
            "musesuperchar",
            "We failed to properly figure out the xTuple Lot/Serial Form Layout.",
            "MuseSuperChar.LotSerial",
            { thrownError: e }
        );

        MuseUtils.displayError(myError, mywindow);
        mywindow.close();
    }
...
})(this.MuseSuperChar.LotSerial, this);
