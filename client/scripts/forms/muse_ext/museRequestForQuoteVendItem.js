// File:        museRequestForQuoteVendItem.js
...
try {
    ...
    if (typeof MuseSuperChar.RfqVendItem === "undefined") {
        MuseSuperChar.RfqVendItem = {};
    }
    ...
    if (
        ...
        var error = new MuseUtils.ScriptException(
            "musesuperchar",
            "We encountered a script level issue while processing MuseSuperChar.RfqVendItem.",
            "MuseSuperChar.RfqVendItem",
            { thrownError: e },
            MuseUtils.LOG_FATAL
        );
        ...
        QMessageBox.critical(
            mainwindow,
            "MuseSuperChar.RfqVendItem Script Error",
            "We encountered a script level issue while processing MuseSuperChar.RfqVendItem."
        );
    }
}
...
(function(pPublicApi, pGlobal) {
    try {
        ...
        var ENTITY_DATA_TABLE = "muserfq_rfqvenditem";
        ...
        var mySave = function(pRecId) {
            ...
            try {
                ...
                var error = new MuseUtils.FormException(
                    ...
                    "MuseSuperChar.RfqVendItem.mySave",
        ...
        var handleGlobalSignal = function(pSource, pMessage) {
            ...
            if (/^_@muserfq@@rfqvenditem@@/.test(pSource)) {
                if (!/^save/.test(pMessage)) {
                    return;
                }

                var targRfqVendItemId = /^_@muserfq@@rfqvenditem@@([0-9]+)/.exec(
                    pSource
                )[1];

                if ((targRfqVendItemId = myId)) {
                    mySave(targRfqVendItemId);
                }
            }
        };
        ...
        pPublicApi.sSignalHandler = function(pSource, pMessage) {
            ...
            try {
                ...
                var error = new MuseUtils.FormException(
                    ...
                    "MuseSuperChar.RfqVendItem.pPublicApi.sSignalHandler",
                    { params: funcParams, thrownError: e },
                    MuseUtils.LOG_CRITICAL
                );
                MuseUtils.displayError(error, mywindow);
        ...
        var error = new MuseUtils.ModuleException(
            "musesuperchar",
            "We enountered a MuseSuperChar.RfqVendItem module error that wasn't otherwise caught and handled.",
            "MuseSuperChar.RfqVendItem",
            { thrownError: e },
            MuseUtils.LOG_FATAL
        );
...
})(MuseSuperChar.RfqVendItem, this);
